...
use Illuminate\Support\Facades\Hash; // ✅ FIX: agregar Hash
...
class UsuariosAdminController extends Controller
{
    public function obtenerTodos(Request $request)
    {
        try {
            ...
            if ($programa) $query->where('programa', $programa);
            if ($search) {
            ...
            $usuarios = $query->select(
                'id', 'name', 'email', 'documento',
                'telefono', 'rol', 'programa',
                'semestre', 'activo', 'created_at'
            )
    ...
    public function obtenerDetalle($usuarioId)
    {
        try {
            ...
            if ($usuario->rol === 'estudiante') {
                $usuario->cursos = DB::table('inscripciones')
    ...
    public function crearUsuario(Request $request)
    {
        try {
            ...
            $usuarioId = DB::table('users')->insertGetId([
                'name'       => $request->name,
                'email'      => $request->email,
                'password'   => Hash::make($request->password), // ✅ FIX: hashear contraseña
    ...
    public function actualizarUsuario(Request $request, $usuarioId)
    {
        try {
            ...
            // ✅ FIX: si mandan nueva contraseña, hashearla
            if ($request->filled('password'))  $datos['password']  = Hash::make($request->password);

            DB::table('users')->where('id', $usuarioId)->update($datos);

            $usuarioActualizado = DB::table('users')->where('id', $usuarioId)->first();
    ...
    public function eliminarUsuario($usuarioId)
    {
        try {
            ...
            // ✅ FIX: proteger también a profesores si es necesario, solo bloquea admins
    ...
    public function obtenerProgramas()
    {
        try {
            ...
            return response()->json(['success' => true, 'data' => $programas], 200);
